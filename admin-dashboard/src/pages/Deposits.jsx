...
import {CreateEditDepositForm} from '../formModals/depositForms/CreateEditDepositForm';
...
export const DepositList = () => {
  ...
  const fetchData = () => {
    depositService.getDeposits().then((data) => {
      const {docs} = data;
      setdata(docs);
      setLoading(false);
    });
  ...
  const updateDeposit = (deposit) => {
    setSelectedUser(deposit);
    setIsEdit(true);
  };

  const deleteDeposit = (depositId, user) => {
    let onOk = async () => {
      const response = await depositService.deleteDeposits(depositId);

      if (response.status !== 'success') {
        toastr.error('Error Handling Request');
      } else {
        const updateDeposit = data?.filter((deposit) => deposit?._id !== depositId);
        toastr.success(response.message);

        setdata(updateDeposit);
      }
    ...
    notifier.confirm('Are you sure ?', onOk, onCancel, {
      labels: {
        confirm: `Delete deposit for ${user?.fullName}`,
      },
    });
  };
  ...
  const listItems = data?.map((deposit, key) => {
    let number = key + 1;
    const user = userList.find((user) => user._id === deposit.user);
    const createdAtDate = new Date(deposit.createdAt).toISOString().slice(0, 10);
    const depositId = deposit._id;
    return (
      <tr key={key}>
        <td>{number++}</td>
        <td>{user ? user.fullName : 'Unknown User'}</td>
        <td>{deposit.depAmount}</td>
        <td>{deposit.photoUrl ? deposit.photoUrl : 'No image available'}</td>
        <td>{createdAtDate}</td>
        <td className="flex align-items-center list-user-action">
          <a
            onClick={() => updateDeposit(deposit)}
          ...
          <a
            onClick={() => deleteDeposit(depositId, user)}
            style={{marginRight: '5px'}}
              ...
              <div className="card-header d-flex justify-content-between">
                <div className="d-flex justify-content-between align-items-center flex-wrap mb-5 gap-3">
                  ...
                  <a
                    ...
                    href="#">
                    <AddButtons title="Create Deposit" />
                  </a>
                </div>
              </div>
              {loading ? (
