...
import {CreateEditBtcForm} from '../../formModals/companyAccountForms/CreateEditBtcForm';
import {useDispatch} from 'react-redux';
import {setBtcList} from '../../redux/user-slice';

export const CompanyBtcs = () => {
  ...
  const fetchData = () => {
    userAccountService.getBtcs().then((data) => {
      ...
      const adminBtcData = docs?.filter((bank) => adminUserIds.includes(bank.user));

      setdata(adminBtcData);
  ...
  const deleteBtc = (btcId, user) => {
    let onOk = async () => {
      const response = await userAccountService.deleteBtcs(btcId);

      if (response.status !== 'success') {
        toastr.error('Error Handling Request');
      } else {
        const updateBtc = data?.filter((btc) => btc?._id !== btcId);
    ...
    notifier.confirm('Are you sure ?', onOk, onCancel, {
      labels: {
        confirm: `Delete btc acount for ${user?.fullName}`,
      },
    });
  };
  ...
  const listItems = data?.map((btc, key) => {
    let number = key + 1;
    const user = userList.find((user) => user._id === btc.user);
    const createdAtDate = new Date(btc.createdAt).toISOString().slice(0, 10);
    const btcId = btc._id;
    return (
      <tr key={key}>
        <td>{number++}</td>
        ...
        <td>
          <tr>{btc.btcWalletAddress}</tr>
          <tr>{btc.btcNetwork}</tr>
          <tr>{createdAtDate}</tr>
        </td>
        <td className="flex align-items-center list-user-action">
          <a
            onClick={() => updateBtc(btc)}
              ...
              <div className="card-header d-flex justify-content-between">
                <div className="d-flex justify-content-between align-items-center flex-wrap mb-5 gap-3">
                  ...
                  <a
                    ...
                    href="#">
                    <AddButtons title="Create BTC Wallet" />
                  </a>
                </div>
              </div>
              {loading ? (
