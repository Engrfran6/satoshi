...
import {CreateEditBankForm} from '../../formModals/companyAccountForms/CreateEditBankForm';
import {setBankList} from '../../redux/user-slice';

export const CompanyBanks = () => {
  ...
  const updateBank = (bank) => {
    setSelectedUser(bank);
    setIsEdit(true);
  };

  const deleteBank = (bankId, user) => {
    let onOk = async () => {
      const response = await userAccountService.deleteBanks(bankId);

      if (response.status !== 'success') {
        toastr.error('Error Handling Request');
      } else {
        const updateBank = data?.filter((bank) => bank?._id !== bankId);
    ...
    notifier.confirm('Are you sure ?', onOk, onCancel, {
      labels: {
        confirm: `Delete bank info's for ${user?.fullName}`,
      },
    });
  };
  ...
  const listItems = data?.map((bank, key) => {
    let number = key + 1;
    const user = userList.find((user) => user._id === bank.user);
    const createdAtDate = new Date(bank.createdAt).toISOString().slice(0, 10);
    ...
    return (
      <tr key={key}>
        ...
        <td>
          <tr>{bank.bankName}</tr>
          <tr>{bank.accountName}</tr>
          <tr>{bank.accountNumber}</tr>
          <tr>{bank.routingNumber}</tr>
          <tr>{bank.bankAddress}</tr>
          <tr>{bank.clientAddress}</tr>
          <tr>{createdAtDate}</tr>
        </td>
        <td className="flex align-items-center list-user-action">
          ...
          <a
            onClick={() => deleteBank(bankId, user)}
            style={{marginRight: '5px'}}
              ...
              <div className="card-header d-flex justify-content-between">
                <div className="d-flex justify-content-between align-items-center flex-wrap mb-5 gap-3">
                  ...
                  <a
                    ...
                    href="#">
                    <AddButtons title="Add bank Account" />
                  </a>
                </div>
              </div>
              {loading ? (
