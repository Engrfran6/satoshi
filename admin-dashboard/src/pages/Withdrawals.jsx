...
import {CreateEditWithForm} from '../formModals/withdrawalForms/CreateEditWithForm';
import {useDispatch} from 'react-redux';
import {setWithdrawalList} from '../redux/user-slice';
...
export const WithdrawalList = () => {
  ...
  const fetchData = () => {
    withdrawalService.getWithdrawals().then((data) => {
      const {docs} = data;
      setdata(docs);
      dispatch(setWithdrawalList(docs));
      setLoading(false);
    });
  };
  ...
  const deleteWithdrawal = (withdrawalId, user) => {
    let onOk = async () => {
      const response = await withdrawalService.deleteWithdrawals(withdrawalId);

      if (response.status !== 'success') {
        toastr.error('Error Handling Request');
      } else {
        const updateWithdrawal = data?.filter((withdrawal) => withdrawal?._id !== withdrawalId);
        toastr.success(response.message);

        setdata(updateWithdrawal);
      }
    ...
    notifier.confirm('Are you sure ?', onOk, onCancel, {
      labels: {
        confirm: `Delete withdrawal request for ${user?.fullName}`,
      },
    });
  };
  ...
  const listItems = data?.map((withdrawal, key) => {
    ...
    let btc = btcList?.find((btc) => btc._id === withdrawal.withToId);
    let usdt = usdtList?.find((usdt) => usdt._id === withdrawal.withToId);

    return (
      <tr key={key}>
        <td>{number++}</td>
        <td>{user ? user.fullName : 'Unknown User'}</td>
        <td>{withdrawal.withAmount}</td>
        <td>
        ...
        <td className="flex align-items-center list-user-action">
          <a
            onClick={() => updateWithdrawal(withdrawal)}
          ...
          <a
            onClick={() => deleteWithdrawal(withdrawalId, user)}
            style={{marginRight: '5px'}}
              ...
              <div className="card-header d-flex justify-content-between">
                <div className="d-flex justify-content-between align-items-center flex-wrap mb-5 gap-3">
                  ...
                  <a
                    ...
                    href="#">
                    <AddButtons title="Create Withdrawal" />
                  </a>
                </div>
              </div>
              {loading ? (
